...
describe("Lit wrapper plugin", () => {
  ...
  beforeAll(async () => {
    sampleMyElement = await fs.readFile(path.resolve(path.join(__dirname, "/../playground/my-element.ts")), {
      encoding: "utf-8"
    });
    samplePage = await fs.readFile(path.resolve(path.join(__dirname, "/../playground/pages/index.vue")), {
      encoding: "utf-8"
    });
    sampleMultiComponentPage = await fs.readFile(
      path.resolve(path.join(__dirname, "/../playground/pages/multiple-different-element-tags.vue")),
      { encoding: "utf-8" }
    );
  });
